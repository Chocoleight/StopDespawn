...
RegisterServerEvent('updateId')
AddEventHandler('updateId', function(oldId, newId)
...
AddEventHandler('save', function(id, model, x, y, z, heading)
	if vehicles[1] then
		for i=1,#vehicles,1 do
...
RegisterServerEvent('retrieveTable')
AddEventHandler('retrieveTable', function()
	dprint('Checking vehicles...')
	TriggerClientEvent('checkVehs', GetPlayers()[1], vehicles)
end)